    ...
    <div class="flex-1 flex items-center justify-center p-6 md:p-12 relative overflow-hidden">
      ...
      <div class="relative w-full max-w-md space-y-8">
        ...
        <Button
          type="button"
          variant="outline"
          class="w-full h-12 font-semibold border-2 hover:bg-accent transition-all group"
...
function handleSpotifyLogin() {
  ...
  // Note: Devise adds /auth prefix, so path is /api/auth/auth/spotify
  const backendUrl = import.meta.env.VITE_API_URL || 'http://127.0.0.1:3000'
  window.location.href = `${backendUrl}/api/auth/auth/spotify`
}
</script>
